  ...
  def draw_graph(self,ax, t_obj):
        ...
        for node in t_obj.tree.all():
          ...
          if node.parent is not None:
            r = 20
            v = node.unit_vec 
            ax.scatter(float(node.parent[0]), float(node.parent[1]), float(node.parent[2]), "-", s=30, color = (0.2, 0.9, 0.8, 0.9), zorder = 5)
            z_axis = v 
            ...
            ax.plot(xyz[0]+node.parent[0], xyz[1]+node.parent[1], xyz[2]+node.parent[2],color='blue') #xyz[0]+node[0], xyz[1]+node[1], xyz[2]+node[2]
            ax.scatter(float(xyz[0][-1]+node.parent[0]), float(xyz[1][-1]+node.parent[1]), float(xyz[2][-1]+node.parent[2]), "-", s=10, color = (0.2, 0.9, 0.2, 0.9), zorder = 5)
            

            tangent = xyz[:, -1] - xyz[:, -2]
            unit_vector = tangent / np.linalg.norm(tangent)
            ax.quiver(xyz[0, -1]+node.parent[0], xyz[1, -1]+node.parent[1], xyz[2, -1]+node.parent[2], unit_vector[0], unit_vector[1], unit_vector[2], length=1.5, color='green')
